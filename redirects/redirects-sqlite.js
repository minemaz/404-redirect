 ...
 *   exact_map  (old_url TEXT PRIMARY KEY, new_url TEXT NOT NULL)
 *   prefix_map (old_prefix TEXT PRIMARY KEY, new_prefix TEXT NOT NULL)
 *     ※ old_prefix には LIKE のメタ文字 % _ を含めないこと
 *   regex_map  (id INTEGER PRIMARY KEY AUTOINCREMENT, pattern TEXT, repl TEXT)
 *
 * JSON 版との比較:
...
(async () => {
  ...
  function lookup(db, path) {
    ...
    const exactHit = queryOne(db,
      'SELECT new_url FROM exact_map WHERE old_url = ? LIMIT 1', variants);
    if (exactHit) return { to: exactHit.new_url, type: 'exact' };
